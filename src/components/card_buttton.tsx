				...
				<div className="relative h-full aspect-[744/1051]">
					...
					<div className="absolute top-[2%] right-[2%] w-15 h-15 sm:w-25 sm:h-25 z-1">
						<Image
							...
							fill
						/>
						<span className="absolute inset-0 flex items-center justify-center text-white text-4xl font-bold rounded sm:pt-1 sm:pr-1.5" style={{ WebkitTextStroke: "0.6px black" }}>
							{data[id].agility}
						</span>
					</div>
						...
						<div className="flex justify-center z-2">
							{data[id].move && data[id].move.when === "pre" && (
								<div className="relative w-35 h-35 sm:w-50 sm:h-50 z-2">
									...
									/>
									<span className={`absolute inset-0 flex items-center justify-center text-white text-4xl font-bold rounded ${data[id].move.type == "free" && "mt-1"} ${data[id].move.type == "away" && "mt-20 sm:mt-30"} ${data[id].move.type == "closer" && "mb-24 sm:mb-32"}`} style={{ WebkitTextStroke: "0.6px black" }}>
										{data[id].move.amnt}
									</span>
								</div>
							)}
							{data[id].damage && (
								<div className="relative w-35 h-35 sm:w-50 sm:h-50 z-2">
									<Image
										...
										className="object-contain"
									/>
									<span className="absolute inset-0 mt-5 sm:mt-8 flex items-center justify-center text-white text-4xl font-bold rounded" style={{ WebkitTextStroke: "0.6px black" }}>
										{data[id].damage}
									</span>
								</div>
							)}
							...
							{data[id].res && (
								<div className="relative w-35 h-35 sm:w-50 sm:h-50 z-2">
									<Image
										...
										className="object-contain"
									/>
									<span className="absolute inset-0 mt-8 flex items-center justify-center text-white text-4xl font-bold rounded" style={{ WebkitTextStroke: "0.6px black" }}>
										{data[id].res.amnt}
									</span>
								</div>
							)}
							...
							{data[id].move && data[id].move.when === "post" && (
								<div className="relative w-35 h-35 sm:w-50 sm:h-50 z-2">
									<Image
										...
										className="object-contain"
									/>
									<span className={`absolute inset-0 flex items-center justify-center text-white text-3xl font-bold rounded ${data[id].move.type == "free" && "mt-1"} ${data[id].move.type == "away" && "mt-20 sm:mt-30"} ${data[id].move.type == "closer" && "mb-24 sm:mb-32"}`} style={{ WebkitTextStroke: "0.6px black" }}>
										{data[id].move.amnt}
									</span>
								</div>
							)}
					...
					{data[id].range && (
						<div>
							<div className={cn("absolute bottom-[2%] left-[2%] w-15 h-15 sm:w-25 sm:h-25", data[id].combo === "0" ? "filter opacity-60" : "no-filter")}>
								<Image
									src="/bg_handler/mh/range.png"
									alt="Range"
									fill
								/>
								<span className="absolute inset-0 flex items-center justify-center text-white text-4xl font-bold rounded" style={{ WebkitTextStroke: "0.6px black" }}>
									{data[id].range}
								</span>
							</div>
							<div className={cn("absolute bottom-[2%] left-[41%] sm:left-[40%] w-15 h-15 sm:w-25 sm:h-25", data[id].break === "0" ? "filter opacity-60" : "no-filter")}>
								<Image
							...
							<div className={cn("absolute bottom-[2%] right-[2%] w-15 h-15 sm:w-25 sm:h-25", data[id].combo === "0" ? "filter opacity-60" : "no-filter")}>
								<Image
